---
title: "Covid 19 dashboard: the case of Sri Lanka"
...
valueBox(
  value = paste(format(sum(df_daily$recovered, na.rm = TRUE), big.mark = ","), " (",
    round(100 * sum(df_daily$recovered, na.rm = TRUE) / sum(df_daily$confirmed), 1),
    "%)",
    sep = ""
  ),
...
```{r}
plotly::plot_ly(data = df_daily, height=300, width=900)%>%
  plotly::add_trace(
    x = ~date,
    # y = ~active_cum,
    y = ~confirmed_cum,
...
htmltools::div(class = "standings",
  ...
  htmltools::h3("Total Number of Covid19 Cases by Country"),
